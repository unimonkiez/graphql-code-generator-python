...
const wrapWithDoubleQuotes: (str: string | number, skipNumericCheck?: boolean) => string = (str, skipNumericCheck) => {
    return `"${str}"`;
}
...
> extends BaseVisitor<TRawConfig, TPluginConfig> {
  ...
  protected getNonExportPrefix(): string {
    return '_';
  }
  ...
  public get scalarsDefinition(): string {
    const allScalars = Object.keys(this.config.scalars).map(scalarName => {
      ...
      return comment + `${this.getNonExportPrefix()}Scalar${scalarName} = ${scalarValue}${this.getPunctuation(scalar)}`;
    });
  ...
  UnionTypeDefinition(node: UnionTypeDefinitionNode, key: string | number | undefined, parent: any): string {
    ...
    return `${this.convertName(node)} = Union[${possibleTypes}]\n`;
  }
  ...
  getObjectTypeDeclarationBlock(
    ...
    const allFields = [
      ...(this.config.addTypename
        ? [
            indent(
              `__typename: ${this.config.nonOptionalTypename ? '' : 'Optional['}Literal["${node.name}"]${this.getPunctuation(
                type
              )}${this.config.nonOptionalTypename ? '' : ']'}`
            ),
          ]
        : []),
      ...node.fields,
    ...
    const declarationBlock = new DeclarationBlock(this._declarationBlockConfig)
      .asKind('class')
      .withName(`${this.convertName(node)}(object)`)
      .withComment((node.description as any) as string);
  ...
  protected _buildTypeImport(identifier: string, source: string, asDefault = false): string {
    if (asDefault) {
      return `import ${source} as ${identifier}`;
    }
  ...
  protected makeValidEnumIdentifier(identifier: string): string {
    if (/^[0-9]/.exec(identifier)) {
      return wrapWithDoubleQuotes(identifier, true);
    }
    return identifier;
  }
    ...
    return values
      .map(enumOption => {
        ...
        return (
          comment +
          indent(
            `${optionName}${this._declarationBlockConfig.enumNameValueSeparator} ${wrapWithDoubleQuotes(
              enumValue,
  ...
  protected _getScalar(name: string): string {
    return `${this.getNonExportPrefix()}Scalar${name}`;
  }
